export const sectoralEnergyRetailArticles = {
  ...
  "elektrik-santrallerinde-ic-tehdit-ve-personel-takibi": {
    ...
    content: `<h2>İçerideki Tehlike: Santral Güvenliğinde İnsan Faktörü</h2><p>Siber saldırıların büyük bir kısmı, dikkatsiz bir personel veya kötü niyetli bir kurum içi aktör tarafından başlatılır. Operatörlerin normal çalışma saatleri ve eriştikleri HMI ekranları SIEM üzerinden rasyonalize edilmelidir.</p><p><strong>ACKLOG Insider Detection:</strong> ACKLOG, operatör hareketlerindeki sapmaları anında raporlar. Manuel takip imkansızdır, ACKLOG ile içerideki gözünüz her zaman açık olsun.</p>`
  },
  ...
  "magaza-ici-pos-ve-yazarkasa-aglari-siber-savunma": {
    ...
    content: `<h2>Kasa Güvenliği: POS Sistemlerini Koruma</h2><p>POS cihazları internete bağlı birer bilgisayardır. Bu cihazlara sızan bir 'Malware', kart verilerini RAM üzerinden okuyup çalabilir.</p><p><strong>ACKLOG POS-Watch:</strong> Tüm şubelerinizdeki POS cihazlarının ağ trafiğini ACKLOG ile merkezi olarak izleyin. Şüpheli bir veri çıkışında ACKLOG o cihazı izole etsin.</p>`
  },
  "zincir-magazalarda-merkezi-log-yonetimi-ve-siem": {
    ...
    content: `<h2>Operasyonel Verimlilik: Şubeler Arası Siber Görünürlük</h2><p>Yüzlerce şubesi olan bir perakende zinciri için her şubedeki teknik sorun veya saldırı merkezi operasyon merkezine anında düşmelidir.</p><p><strong>ACKLOG Multi-Branch SIEM:</strong> ACKLOG, dağıtık mimarisi ile yüzlerce şubenizi merkezi olarak yönetmenizi sağlar.</p>`
  },
  "musteri-hizmetleri-call-center-log-analizi-ve-guv": {
    ...
    content: `<h2>Çağrı Merkezi Güvenliği: Ses ve Veri Entegrasyonu</h2><p>Operatörlerin müşteri kartlarındaki 'Düzenleme' hareketleri ve ses kayıt dosyalarının download logları mutlaka tutulmalıdır.</p><p><strong>ACKLOG Call-Center Monitor:</strong> Çağrı merkezi sistemlerinizdeki her bir hareketi ACKLOG ile mühürleyin. Suiistimalleri gerçekleştiği anda loglardan yakalayın.</p>`
  },
  "perakende-zincirlerinde-envanter-yonetimi-ve-siber-risk": {
    ...
    content: `<h2>Stok Sabotajı: Envanter Verilerini Kim Değiştiriyor?</h2><p>E-ticaret sitelerinde stok miktarının değiştirilmesi bir siber saldırı türüdür. Bu işlemler ancak veritabanı loglarında izlenebilir.</p><p><strong>ACKLOG Inventory Shield:</strong> Ticari verilerinizi ACKLOG ile koruyun. 'Bir kerede 1000 ürünün fiyatı değişti' gibi anomalileri yakalayın.</p>`
  },
  "magaza-ici-hirsizlik-ve-kayip-onleme-siber-boyut": {
    ...
    content: `<h2>Kayıp Önleme: Fiziksel ve Dijitalin Kesişimi</h2><p>Mağaza içindeki ürünlerin 'kaybolması' sadece hırsızlık değil, sistem üzerindeki stok manipülasyonu ile de olabilir. Kamera (CCTV) logları ile stok değişim logları SIEM üzerinde birleştirilmelidir.</p><p><strong>ACKLOG Retail-Safe:</strong> Mağazalarınızdaki fiziksel ve dijital olayları tek ekranda izleyin. Şüpheli durumlarda ACKLOG ile kesin kanıtlar oluşturun.</p>`
  },
  "sadakat-programlari-loyalty-guvenligi-ve-fraud-tespiti": {
    ...
    content: `<h2>Puan Hırsızlığı: Sadakat Programlarının Korunması</h2><p>Müşterilerin biriktirdiği puanlar 'Dijital Para'dır. Bir saldırganın müşterilerin puanlarını başka hesaplara aktarması veya sahte puan üretmesi loglardan yakalanabilir.</p><p><strong>ACKLOG Loyalty-Watch:</strong> Sadakat programı API'lerinizi ACKLOG ile izleyin. Anormal puan kazanımı ve harcama paternlerini anında saptayın.</p>`
  },
