...
export async function generateStaticParams() {
  return Object.keys(checklistPages).map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const slug = (await params).slug;
  const data = checklistPages[slug];
  if (!data) return { title: "Sayfa Bulunamadı" };
  return {
    title: data.title, description: data.description,
    alternates: { canonical: `https://logsiem.com/checklist/${slug}` },
    openGraph: { title: data.title, description: data.description, url: `https://logsiem.com/checklist/${slug}`, type: 'article', locale: 'tr_TR', siteName: 'ACKLOG SIEM' },
  };
}
...
export default async function ChecklistPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const articleSchema = getArticleSchema(data.title, data.description, `https://logsiem.com/checklist/${slug}`, publishDate);
  ...
  return (
    <>
      ...
      <SeoTemplate data={data} backText="Uyum Çözümleri" backUrl="/cozumler" />
    </>
  );
}
