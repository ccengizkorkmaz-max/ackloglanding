...
export async function generateStaticParams() {
  return Object.keys(attackTypePages).map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const slug = (await params).slug;
  const data = attackTypePages[slug];
  if (!data) return { title: "Sayfa Bulunamadı" };
  return {
    title: data.title, description: data.description,
    alternates: { canonical: `https://logsiem.com/saldiri/${slug}` },
    openGraph: { title: data.title, description: data.description, url: `https://logsiem.com/saldiri/${slug}`, type: 'article', locale: 'tr_TR', siteName: 'ACKLOG SIEM' },
  };
}

export default async function AttackTypePage({ params }: { params: Promise<{ slug: string }> }) {
  const slug = (await params).slug;
  const data = attackTypePages[slug];
  if (!data) notFound();

  const publishDate = '2026-06-10T10:00:00+03:00';
  const articleSchema = getArticleSchema(data.title, data.description, `https://logsiem.com/saldiri/${slug}`, publishDate);
