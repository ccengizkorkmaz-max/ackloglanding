...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const data = useCasePages[slug];
  if (!data) return { title: "Sayfa Bulunamadı" };
  return {
    title: data.title, description: data.description,
    alternates: { canonical: `https://logsiem.com/kullanim/${slug}` },
    openGraph: { title: data.title, description: data.description, url: `https://logsiem.com/kullanim/${slug}`, type: 'article', locale: 'tr_TR', siteName: 'ACKLOG SIEM' },
  };
}
...
export default async function UseCasePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const articleSchema = getArticleSchema(data.title, data.description, `https://logsiem.com/kullanim/${slug}`, publishDate);
