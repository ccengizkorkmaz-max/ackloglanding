...
import { programmaticCities } from "@/data/programmatic-seo";
import { SeoTemplate } from "@/components/seo-template";

export async function generateStaticParams() {
  return Object.keys(programmaticCities).map((slug) => ({
    slug,
  }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const data = programmaticCities[slug];
  ...
  return {
    ...
    alternates: {
      canonical: `https://logsiem.com/sehir/${slug}`,
    },
    openGraph: {
      title: data.title,
      description: data.description,
      url: `https://logsiem.com/sehir/${slug}`,
      type: 'article',
      locale: 'tr_TR',
...
export default async function CitySeoPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const articleSchema = {
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `https://logsiem.com/sehir/${slug}`
    },
    "headline": data.title,
    "description": data.description,
    "author": {
